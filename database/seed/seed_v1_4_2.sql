-- Material Masterdata Portal V1.4.2 demo data
-- Idempotent: safe to run multiple times on an existing local database.
...
(
 ...
 'Vải thun cotton 2 chiều màu đen',
 'Vải thun cotton 2 chiều, màu đen, khổ 1.8m, định lượng 180gsm',
 'KG','FABRIC','PENDING_MASTERDATA',NULL,NULL,NULL,
 NOW() - INTERVAL '6 hours', NOW() - INTERVAL '6 hours'
),
(
 'REQ-DEMO-0002',
 (SELECT id FROM users WHERE email='user2@example.com'),
 'Khóa kéo YKK số 3 màu navy',
 'Khóa kéo YKK #3 màu navy dùng cho áo khoác',
 'PCS','ACCESSORY','PENDING_ACCOUNTING','Đã kiểm tra trùng mã, chưa có vật tư tương đương',NULL,NULL,
 NOW() - INTERVAL '1 day', NOW() - INTERVAL '3 hours'
),
(
 'REQ-DEMO-0003',
 (SELECT id FROM users WHERE email='user@example.com'),
 'Tem size dệt S-M-L-XL',
 'Tem size dệt polyester bộ 4 size',
 'PCS','LABEL','PENDING_CODE_ASSIGNMENT','Thông tin kỹ thuật phù hợp','Đã xác nhận nhóm tài khoản vật tư',NULL,
 NOW() - INTERVAL '2 days', NOW() - INTERVAL '4 hours'
),
(
 'REQ-DEMO-0004',
 (SELECT id FROM users WHERE email='user2@example.com'),
 'Túi zipper PE 30x40 cm',
 'Túi zipper trong 30x40cm dùng đóng gói phụ kiện',
 'PCS','PACKING','RETURNED_TO_REQUESTER','Cần bổ sung độ dày túi và quy cách đóng gói',NULL,NULL,
 NOW() - INTERVAL '3 days', NOW() - INTERVAL '1 hour'
),
(
 'REQ-DEMO-0005',
 (SELECT id FROM users WHERE email='user@example.com'),
 'Nhãn hướng dẫn giặt 4x8 cm',
 'Nhãn satin in hướng dẫn giặt',
 'PCS','LABEL','COMPLETED','Đã duyệt','Đã duyệt','VT000101',
...
WHERE NOT EXISTS (
    SELECT 1 FROM material_requests r WHERE r.request_no = v.request_no
);
...
SELECT r.id, u.id, 'CREATE_REQUEST', NULL, r.status,
       'Dữ liệu mẫu phục vụ kiểm thử Material Masterdata Portal V1.4.2', r.created_at
FROM material_requests r
JOIN users u ON u.email = CASE
    WHEN r.request_no IN ('REQ-DEMO-0002','REQ-DEMO-0004') THEN 'user2@example.com'
    ELSE 'user@example.com'
END
WHERE r.request_no LIKE 'REQ-DEMO-%'
  AND NOT EXISTS (
      SELECT 1 FROM request_history h
      WHERE h.request_id = r.id AND h.action = 'CREATE_REQUEST'
  );

-- Add realistic workflow history for the returned demo request.
INSERT INTO request_history (request_id, actor_id, action, from_status, to_status, note, created_at)
...
WHERE r.request_no='REQ-DEMO-0004'
  AND NOT EXISTS (
      SELECT 1 FROM request_history h
      WHERE h.request_id=r.id AND h.action='RETURN'
  );
